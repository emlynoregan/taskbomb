...
#     logging.warning("mapper_spec: %s" % mapper_spec)
    logging.warning("mapper_spec.params: %s" % mapper_spec.params)
    message = (
...
class GoogleCloudStorageConsistentOutputWriter(
  ...
  def create(cls, mr_spec, shard_number, shard_attempt, _writer_state=None):
    """Inherit docs."""
    logging.warning("mr_spec.mapper: %s" % mr_spec.mapper)
    writer_spec = cls.get_params(mr_spec.mapper, allow_old=False)
